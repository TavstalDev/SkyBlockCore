...
// Note: This is a very basic implementation of the GUI.
// I am probably going to make a custom solution in my library using SpiGUI as a base.
// Since many menus are not exactly a good solution for my current caching solutions.
// Also, they are too static, and it is not easy to make dynamic content with pagination, etc.
public class FactoriesMainMenu {
    private static final PluginLogger _logger = SkyBlockCore.logger().withModule(FactoriesMainMenu.class);
    ...
    public static SGMenu create(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to create Factory Main Menu for player " + player.getName());
            _logger.error(ex);
            return null;
        }
    }
    ...
    public static void refresh(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to refresh Factory Main Menu for player " + player.getName());
            _logger.error(ex);
        }
    }
}
