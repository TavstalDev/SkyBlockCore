...
public class JobsEarningMenu {
    private static final PluginLogger _logger = SkyBlockCore.logger().withModule(JobsEarningMenu.class);
    ...
    public static SGMenu create(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to create Jobs Earning Menu for player " + player.getName());
            _logger.error(ex);
            return null;
        }
    }
    ...
    public static void refresh(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to refresh Jobs Earning Menu for player " + player.getName());
            _logger.error(ex);
        }
    }
}
