...
public class JobsLevelMenu {
    private static final PluginLogger _logger = SkyBlockCore.logger().withModule(JobsLevelMenu.class);
    ...
    public static SGMenu create(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to create Jobs Level Menu for player " + player.getName());
            _logger.error(ex);
            return null;
        }
    }
    ...
    public static void refresh(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to refresh Jobs Level Menu for player " + player.getName());
            _logger.error(ex);
        }
    }
}