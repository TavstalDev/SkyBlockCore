...
public class JobsPointShop {
    private static final PluginLogger _logger = SkyBlockCore.logger().withModule(JobsPointShop.class);
    ...
    public static SGMenu create(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to create Jobs Point Shop Menu for player " + player.getName());
            _logger.error(ex);
            return null;
        }
    }
    ...
    public static void refresh(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to refresh Jobs Point Shop Menu for player " + player.getName());
            _logger.error(ex);
        }
    }
}
