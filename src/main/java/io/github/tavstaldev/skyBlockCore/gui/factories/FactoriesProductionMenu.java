...
public class FactoriesProductionMenu {
    private static final PluginLogger _logger = SkyBlockCore.logger().withModule(FactoriesProductionMenu.class);
    ...
    public static SGMenu create(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to create Factory Production Menu for player " + player.getName());
            _logger.error(ex);
            return null;
        }
    }
    ...
    public static void refresh(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to refresh Factory Production Menu for player " + player.getName());
            _logger.error(ex);
        }
    }
}
