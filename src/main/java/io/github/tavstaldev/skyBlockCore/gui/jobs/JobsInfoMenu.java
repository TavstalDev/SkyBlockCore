...
public class JobsInfoMenu {
    private static final PluginLogger _logger = SkyBlockCore.logger().withModule(JobsInfoMenu.class);
    ...
    public static SGMenu create(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to create Jobs Info Menu for player " + player.getName());
            _logger.error(ex);
            return null;
        }
    }
    ...
    public static void refresh(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to refresh Jobs Info Menu for player " + player.getName());
            _logger.error(ex);
        }
    }
}
