...
public class FactoriesRewardsMenu {
    private static final PluginLogger _logger = SkyBlockCore.logger().withModule(FactoriesRewardsMenu.class);
    ...
    public static SGMenu create(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to create Factory Rewards Menu for player " + player.getName());
            _logger.error(ex);
            return null;
        }
    }
    ...
    public static void refresh(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to refresh Factory Rewards Menu for player " + player.getName());
            _logger.error(ex);
        }
    }
}
