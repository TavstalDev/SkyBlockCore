...
public class JobsMainMenu {
    private static final PluginLogger _logger = SkyBlockCore.logger().withModule(JobsMainMenu.class);
    ...
    public static SGMenu create(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to create Jobs Main Menu for player " + player.getName());
            _logger.error(ex);
            return null;
        }
    }
    ...
    public static void refresh(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to refresh Jobs Main Menu for player " + player.getName());
            _logger.error(ex);
        }
    }
}
