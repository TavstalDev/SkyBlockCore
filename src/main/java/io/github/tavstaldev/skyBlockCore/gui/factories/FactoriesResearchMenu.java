...
public class FactoriesResearchMenu {
    private static final PluginLogger _logger = SkyBlockCore.logger().withModule(FactoriesResearchMenu.class);
    ...
    public static SGMenu create(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to create Factory Research Menu for player " + player.getName());
            _logger.error(ex);
            return null;
        }
    }
    ...
    public static void refresh(@NotNull Player player) {
        try {
            ...
            _logger.error("Failed to refresh Factory Research Menu for player " + player.getName());
            _logger.error(ex);
        }
    }
}
